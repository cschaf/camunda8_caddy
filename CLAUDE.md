...
- Docker Compose loads both files: `start.sh` / `start.ps1` pass `--env-file .env --env-file .env-credentials` (so `${VAR}` interpolation in `docker-compose.yaml` works), and the credential-relevant services additionally have `env_file: - path: .env-credentials / required: true` in `docker-compose.yaml` (defense in depth — `docker compose up` without the start scripts still works).
